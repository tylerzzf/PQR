twobreak.sim.func <- function(n,T,p,taus,cpt.num,nrep,eps,cpt.eps,step.max){
  ptm <- proc.time();
  ...
  for( i in c(1:nrep)){
    ...
    X = data.tem$X
    beta = data.tem$beta
    cpt0 = c(data.tem$cpt.1,data.tem$cpt.2)
    
    lambda.range = seq(1,2.5,length.out=20)
    ...
    if(length(cpt.1) != 0){
      distance = hausdorff.func(cpt.1,cpt0)
      distance = 100*distance/T
      if(distance!=0){
    ...
    if(length(cpt.2) != 0){
      distance = hausdorff.func(cpt.2,cpt0)
      distance = 100*distance/T
      if(distance!=0){
    ...
    if(length(cpt.3) != 0){
      distance = hausdorff.func(cpt.3,cpt0)
      distance = 100*distance/T
      if(distance!=0){
